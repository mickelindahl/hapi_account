...
async function _onPreAuth( request, h ) {
    ...
    if ( cookies[token_name] == undefined ) {

        debug('_onPreAuth', request.path,' not valid cookie');

        return h.continue;
    }

    request.headers.authorization = 'Bearer ' + cookies[token_name];

    debug( '_onPreAuth auth',  request.path, request.headers.authorization );
