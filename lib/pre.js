...
function verifyUser( request, reply ) {
    ...
    request.pre.accountDatabase.findOne( criteria ).then( account => {
        ...
        debug( 'verifyUser', keyId, account[keyId] );
 ...
 *
 */
function verifyOrCreateExternalUser( created_by, request, reply ) {
    ...
    request.pre.accountDatabase.findOne( criteria ).then( account => {
        ...
        if ( !account ) {
            ...
            request.pre.accountDatabase.create(new_account).then(account=> {
                ...
                debug( 'verifyOrCreateExternalUser created', keyId, account[keyId] );
            ...
            debug( 'verifyOrCreateExternalUser verified', keyId, account[keyId] );
            reply( account );
