...
function verifyOrCreateFacebookUser( request, reply ) {
    ...
    request.pre.accountDatabase.findOne( criteria ).then( account => {
        ...
        debug( 'verifyOrCreateFacebookUser', account )

        if ( !account ) {

            request.pre.accountDatabase.create({
                ...
                debug('verifyOrCreateFacebookUser facebook account created');
            ...
            if ( account.type != 'facebook' ) {
                return reply( boom.badRequest( 'Not a facebook account, account already taken' ) );
            }
