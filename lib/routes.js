 ...
 * - `{options.basePath}/createOrLoginFacebook` {POST}
 * - `{options.basePath}/createOrLoginGoogle` {POST}
...
function routes(config, pre, options) {
    return [
        ...
        {method: 'POST', path: pre + options.basePath + '/createOrLoginFacebook', config: config.createOrLoginFacebook},
        {method: 'POST', path: pre + options.basePath + '/createOrLoginGoogle', config: config.createOrLoginGoogle},
        {method: 'POST', path: pre + options.basePath + '/login', config: config.login},
        {method: 'POST', path: pre + options.basePath + '/loginFacebook', config: config.loginFacebook},
        {method: 'POST', path: pre + options.basePath + '/loginGoogle', config: config.loginGoogle},
        {method: 'POST', path: pre + options.basePath + '/logout', config: config.logout},
        {method: 'POST', path: pre + options.basePath + '/renewToken', config: config.renewToken},
        {method: 'POST', path: pre + options.basePath + '/resetPassword', config: config.resetPassword},
        {method: 'POST', path: pre + options.basePath + '/updateScope', config: config.updateScope},
