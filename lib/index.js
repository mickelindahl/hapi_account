...
function expiredTokenCollector() {
    ...
    let date =  new Date()
    let criteria = { expireAt: { '<': date} }
    debug( 'expiredTokenCollector', date );

    // Need to have then to work
    this.destroy( criteria ).then( t => {
...
function _getCronJobs( server, options ) {
    ...
    let _cronTime = options.cronTime.ExpiredTokenCollector || '00 * * * * *';
 ...
 *     For each call just inspect respective data object to see what data is available.
 ...
 *     - `create` Duration (in miliseconds) the  token created in `create` route is valid
 *     - `login` Duration (in miliseconds) the  token created in `login` route is valid
...
exports.register = function ( server, options, next ) {
    ...
    setTimeout(()=>{
        ...
        debug('Start expire toeken cron jobs')
