...
function _get_config( method, options ) {
    let config = {};
    ...
    config.updateScope = {
        ...
        tags: ['api', 'account'],
        handler: method.handlerUpdateScope
    };
 ...
 * @api public
 */
function handlerUpdateScope( request, reply ) {

    debug( 'handlerSetScope' );
    ...
    Account.find(criteria).then(account=>{
        ...
        scope=config.add.reduce((tot, val)=>{
            ...
            debug('add',tot.indexOf(val), val, tot)
        ...
        return _doEvents( request, 'onPostUpdateScope' )
...
let method = {
    ...
    'handlerUpdateScope': handlerUpdateScope,
...
exports.register = function ( server, options, next ) {
    ...
    let config = _get_config( _attachMethods( options.method, method ), options );
