...
function attachPostEmailEvents( emails, events) {
        ...
        emails.events.forEach(e=>{
            ...
            event.push({
                ...
                method: (request, next)=>{
                    ...
                    let  transporter = request.server.plugins['hapi-account'].transporter;

                    transporter.sendMail({
                        from:e.from,
                        to: request.pre.accountEntry.email,
                        subject: e.subject,
                        text: typeof e.test == 'function'
                            ? e.test(request)
                            : e.test,
...
exports.register = function ( server, options, next ) {
    ...
    options.events = attachPostEmailEvents(options.email, options.events)
    // _options = options;
