...
function handlerLogin( request, reply ) {
    ...
    request.pre.tokenDatabase.findOne( criteria ).then( token=> {
        ...
        let id_token = Jwt.sign( token, process.env.JWT_SECRET, {
            algorithm: 'HS256',
            expiresIn: "12h"
        } );
        ...
        reply( { token: token } ).header( 'Set-Cookie', cookie).code(201);
...
function verifyPassword( request, reply ) {
    ...
    request.pre.accountDatabase.findOne( criteria ).then( account => {
        ...
        bcrypt.compare( request.payload.encrypted_password, account.password, function ( err, res ) {
...
function _get_config( methods ) {
    ...
    config.create = {
        pre: [
            { method: methods.getDatabaseAccount, assign: 'accountDatabase' },
            { method: methods.isAccount },
            { method: methods.encryptPassword },
            { method: methods.getDatabaseToken, assign: 'tokenDatabase' },
        ],
        validate: {
    ...
    config.login = {
        pre: [
            { method: methods.getDatabaseAccount, assign: 'accountDatabase' },
            { method: methods.verifyUser, assign: 'accountEntry' },
            { method: methods.verifyPassword },
            { method: methods.getDatabaseToken, assign: 'tokenDatabase' },
        ],
        validate: {
            payload: Joi.object().keys( {
    ...
    config.resetPassword = {
        pre: [
            { method: methods.getDatabaseToken, assign: 'tokenDatabase' },
            { method: methods.verifyToken, assign: 'tokenEntry' },
            { method: methods.destroyToken },
            { method: methods.encryptPassword },
            { method: methods.getDatabaseAccount, assign: 'accountDatabase' }],
        validate: {
    ...
    config.verifyAccount = {
        pre: [
            ...
            { method: methods.verifyToken, assign: 'tokenEntry' },
            { method: methods.destroyToken },
            { method: methods.getDatabaseToken, assign: 'accountDatabase' },
        ],
...
exports.register = function ( server, options, next ) {
    ...
    methods=_attachMethods(options.methods, methods);
