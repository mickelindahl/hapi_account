...
function destroyToken( request, reply) {
    ...
    request.pre.token.destroy({ uuid:token}).then(token=>{
...
function handlerCreate( request, reply ) {
    ...
    account.verified = request.server.plugins['hapi-account'].verfiyAccount ?
        true:
        false;
    ...
    request.pre.account.create( account ).then( account=> {
        ...
        debug('handlerCreate error', err.message )
...
function handlerLogin( request, reply ) {
    ...
    request.pre.account.findOne( { user: request.payload.user } ).then( account => {
